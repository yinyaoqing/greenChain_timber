...
| AT-5 上鏈重試 | 引用 M3 完整演練（本機，同一程式碼與正式 DB） | ✅ PASS | 壞 RPC → 3 次重試失敗、狀態停留 chain_pending、last_error=「所有 RPC 節點皆無法連線」→ 恢復 RPC + `POST /api/admin/retry-pending` → 5 秒轉 on_chain（token #4，tx `0x12ba82...4b5a52`）；詳 devlog 2026-08-07 M3 段 |
