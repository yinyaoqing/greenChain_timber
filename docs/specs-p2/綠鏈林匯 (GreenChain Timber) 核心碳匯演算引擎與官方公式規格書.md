# 核心碳匯演算引擎與計量公式規格書（第二階段草案）

| 文件資訊 | 內容 |
|---|---|
| 文件性質 | **第二階段（DBH 精確模式）引擎規格草案** |
| 狀態 | 草案｜對外：否｜最後校訂：2026-08-25 |
| 修訂紀錄 | 2026-08-25：去官方化（標題與內文移除「官方指定」表述）、主管機關口徑修正（環境部方法學為架構、農業部提供係數與材積式）、formula_version 改為自有版號 GCT-ARR-2026-draft |
| 與現行系統關係 | 現行 v1.0 引擎（樹種/林齡/密度輸入，`MOA-2024-v1`，已上線）**不受本文件影響**；本文件規範 SBIR 第二階段之「胸徑（DBH）輸入模式」，兩模式並存（年齡式＝快速估算、DBH 式＝實測精確估算） |
| 係數狀態 | 本文件所列 D／BEF／R 與材積式係數為**候選值**，正式定稿前須補齊文獻頁碼層級之可稽核引註（對應 SBIR 第二階段研發項目 1）。查證定稿排程：2027年度 SBIR 計畫期程內 |
...
### 3. 二元材積式（候選值，待查證）
計算單棵樹材積（\(V\)）時，系統採用二元材積式（胸徑 \(DBH\) 單位為公分；樹高 \(H\) 單位為公尺）：
\[V = a \times (DBH)^b \times H^c\]
*   **台灣杉 (Taiwania)**：\(V = 0.000068 \times (DBH)^{1.852} \times H^{0.925}\)［待補：文獻/彙編版本與頁碼］
*   **相思樹 (Acacia)**：\(V = 0.000075 \times (DBH)^{1.902} \times H^{0.855}\)［待補：文獻/彙編版本與頁碼］
*   **光臘樹 (Fraxinus)**：官方立木材積表**未列**光臘樹之二元求積式，依《樹種字典規格書》採**通用形數法**：\(V = (DBH/100)^2 \times 0.7854 \times H \times 0.45\)（查證時若確認官方另列有迴歸式，再以查證結果為準）

> **與《樹種字典規格書》之分工（避免循環引用）**：兩份文件出入時，**D／BEF／R／CF 係數以本規格書為準；材積式（求積式選型與係數）以《樹種字典規格書》為準**。
...
OFFICIAL_SPECIES_COEFFICIENTS = {
    ...
    "fraxinus": {  # 光臘樹（官方材積表未列迴歸式，採通用形數法 f=0.45，見《樹種字典規格書》）
        "name": "光臘樹",
        "density": 0.580,
        "bef": 1.48,
        "root_ratio": 0.26,
        "carbon_fraction": 0.5,
        "volume_formula": lambda dbh, h: ((dbh / 100.0) ** 2) * 0.7854 * h * 0.45
    }
}
...
def calculate_forest_carbon_sink(payload: CarbonCalculationPayload) -> dict:
    """
    以環境部《造林與植林碳匯專案活動方法學》為架構、採農業部公告係數（現行為文獻示範參數）計算森林二氧化碳吸存量
    """
    ...
    return {
        ...
        "formula_version": "GCT-ARR-2026-draft",  # 候選係數查證定稿後改為正式版號並凍結
    }
```
